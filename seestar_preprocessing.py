...
class PreprocessingInterface:
        ...
        if os.path.isdir(directory):
            ...
            if file_count > 2048:
                self.siril.error_messagebox(
                    "More than 2048 images found. Please select '2048+ Mode' and try again."
                )
                self.close_dialog()
            ...
            try:
                if self.fitseq_mode:
                    if not self.drizzle_status:
                        ...
                        self.siril.cmd(
                            "convert", dir_name, "-fitseq", "-out=../process"
                        )
                else:
                    if not self.drizzle_status:
                        self.siril.cmd(
                            "convert", dir_name, "-debayer", "-out=../process"
                        )
                    else:
    ...
    def seq_apply_reg(self, seq_name, drizzle_amount, pixel_fraction):
        ...
        if self.fitseq_mode:
            cmd_args = ["register", seq_name, "-2pass", "-selected"]
    ...
    def spcc(
        ...
        if oscsensor in {"ZWO Seestar S30", "ZWO Seestar S50"}:
            if filter == "broadband":
                ...
                args.append("-oscfilter=ZWO Seestar LP")
        elif oscsensor == "Dwarf 3":
            if filter == "broadband":
                args.append("-oscfilter=Sony IMX678")
            elif filter == "narrowband":
                args.extend(
                    [
                        "-narrowband",
    ...
    def autostretch(self, do_spcc):
        ...
        self.siril.log(
            "Autostretched image." + (" You may want to open the _spcc file instead." if do_spcc else ""),
            LogColor.GREEN,
        )
    ...
    def create_widgets(self):
        ...
        if self.gaia_catalogue_available:
            ...
            ttk.Label(
                spcc_section, text="✗ Local Gaia Not available", foreground="red"
            ).grid(row=2, column=2, sticky="w")
